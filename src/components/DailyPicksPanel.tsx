...
function DayCard({ d }: { d: any }) {
  ...
  return (
    <div className="glass rounded-xl p-4 space-y-4 min-w-0">
      ...
      <div className="pt-2 border-t border-border/60">
        <div className="flex flex-wrap items-center gap-1.5 text-[10px] uppercase tracking-widest text-muted-foreground mb-1.5">
...
export function DailyPicksPanel() {
  ...
  return (
    <section className="mx-auto w-full max-w-6xl px-3 sm:px-4 lg:px-6 pb-10 sm:pb-12">
      <div className="mb-4">
        <h2 className="text-xl font-bold">Single & Combo <span className="text-neon">of the Day</span></h2>
        <p className="text-xs text-muted-foreground mt-1 max-w-4xl leading-5">
      ...
      {q.isLoading ? (
        ...
        <div className="glass rounded-xl p-6 sm:p-8 text-center text-sm text-muted-foreground">No qualifying picks yet — run a scan.</div>
      ) : (
      ...
      {totalPages > 1 && (
        <div className="mt-4 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
          ...
          <div className="flex w-full sm:w-auto gap-2">
            <Button className="flex-1 sm:flex-none" variant="outline" size="sm" disabled={page <= 1} onClick={() => setPage((p) => Math.max(1, p - 1))}>
              <ChevronLeft className="h-4 w-4" /> Previous
            </Button>
            <Button className="flex-1 sm:flex-none" variant="outline" size="sm" disabled={page >= totalPages} onClick={() => setPage((p) => Math.min(totalPages, p + 1))}>
